...
class Ajax extends Backend
{
    ...
    public function isHaveNewAfter()
    {
        ...
        $recharge_count = RechargeRecord::where(['status'=>0])->count();
        $withdraw_count = UserWithdraw::where(['status'=>0])->count();
        if($recharge_count > 0){
            $this->success("有新的充值订单");
        }elseif($withdraw_count > 0){
             $this->success("有新的提现订单");
        }else{
            $this->error("暂无订单");
        }
    }    
