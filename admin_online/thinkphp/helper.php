...
if (!function_exists('prefix_image')) {
    /**
     * 图片加上域名
     * @param string $path 路径
     * @return string
     */
    function prefix_image($path){
        $domain = Request::instance()->domain();
        if(strlen($path) > 4 && strpos($path,'http') !== 0){
            $path = $domain.$path;
        }
        return $path;
    }
...
if (!function_exists('get_usdt_rate')) {
    /**
     * 获取usdt汇率
     * @param string $coin 币种
     * @return string
     */
    function get_usdt_rate(string $coin){
        $redis = get_redis();
        $redis->select(12);
        $detail = json_decode($redis->get("huobi:ticker:".strtolower($coin . 'usdt')),true);
        return isset($detail['close']) ? sprintf("%.6f", $detail['close']) : db('wallet_coin')->where('coin', $coin)->value('rate');
    }
}
...
if (!function_exists('http_curl')) {
     ...
     * @param   string  $authorization  请求头部
     * @param   int     $timeout        超时时间
     * @return  mixed
     */
    function curl_request($url,  $data = '', $type = 'get', $authorization = '',$timeout = 30)
    {
        $curl = curl_init();
        $array = [
            CURLOPT_URL => $url,
        ...
        if ($type == 'get') unset($array['CURLOPT_POSTFIELDS']);
        curl_setopt_array($curl, $array);
        $response = curl_exec($curl);
        $err = curl_error($curl);
        curl_close($curl);
        if ($err) \think\Log::error($err);
        return $response;
    }
}
...
if (!function_exists('lock')) {
     ...
     * @return false|string
     */
    function lock(string $key, int $expire = 5, int $num = 2)
    {
        $redis = get_redis();
        $isLock = $redis->set($key, time() + $expire, ["NX", 'EX' => $expire]);
        if(!$isLock) {
            //获取锁失败则重试{$num}次
            for($i = 0; $i < $num; $i++){
                $isLock = $redis->set($key,time() + $expire, ["NX",'EX'=>$expire]);;
